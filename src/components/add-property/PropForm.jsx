...
import FetchData from "@/api/add-property/FetchData";
...
export default function AntdForm() {
  ...
  const validate = () => {
    ...
    if (
      bedrooms &&
      (!numericOnlyRegex.test(bedrooms) || parseInt(bedrooms) < 0)
    ) {
      newErrors.bedrooms = "Field must contain only positive numbers.";
    }
    if (
      bathrooms &&
      (!numericOnlyRegex.test(bathrooms) || parseInt(bathrooms) < 0)
    ) {
      newErrors.bathrooms = "Field must contain only positive numbers.";
    }
    if (
      parkingSpaces &&
      (!numericOnlyRegex.test(parkingSpaces) || parseInt(parkingSpaces) < 0)
    ) {
      newErrors.parkingSpaces = "Field must contain only positive numbers.";
    }
    if (
      surfaceArea &&
      (!numericOnlyRegex.test(surfaceArea) || parseInt(surfaceArea) < 0)
    ) {
    ...
    if (validate()) {
      ...
      if (imagePaths.length === 0) {
        ...
        for (let imagePath of imagePaths) {
          imageUrls.push(
            `https://kpcjtdxeopfbjrzubivj.supabase.co/storage/v1/object/public/images/${imagePath}`
          );
        }
      }
      let propertyData = {
        ...
        <Form layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item>
            <Button
              ...
              htmlType="submit"
              block
              className="mt-2 bg-indigo-500 hover:bg-indigo-600 active:bg-indigo-500"
            >
              Submit
            </Button>
          </Form.Item>
